...
void print_extended_branch_stats(){
    // MY CODE
    // print each branch
    for(int i = 0; i < NUM_CPUS; i++){
        cout << "UNIQUE BRANCHES: " << branches_map[i].size() << endl;;
        cout << "STATS FOR EACH BRANCH" << endl;
        for(auto it = branches_map[i].begin(); it != branches_map[i].end(); it++){
            fprintf(stdout, "ip:%10p\t\ttimes-executed:%10d\t\ttaken:%10d\t\tnot-taken:%10d\t\tcorrpred:%10d\t\tmispred:%10d\t\tmispred-rate%10.3f%%\n", (void *)it->first, it->second->times_executed, it->second->taken, it->second->times_executed - it->second->taken, it->second->times_executed - it->second->mispred, it->second->mispred, 100.0 * it->second->mispred / it->second->times_executed);
            free(it->second);
        }
    }
