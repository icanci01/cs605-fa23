...
// MY CODE
map<uint64_t, branch_stats*> branches_map[NUM_CPUS];
// END OF MY CODE
	...
	else
	  {
		...
		if((arch_instr.is_branch == 1) && (arch_instr.branch_taken == 1))
		  ...
		  num_reads++;
                    ...
                    if (IFETCH_BUFFER.entry[ifetch_buffer_index].is_branch) {
                         // MY CODE
                        branch_stats* current_branch = NULL;
                        if(arch_instr.branch_type == BRANCH_CONDITIONAL) {
                           
                            if(warmup_complete[cpu]){

                                auto it = branches_map[cpu].find(arch_instr.ip);

                                if (it == branches_map[cpu].end()){
                                    branch_stats* temp = (branch_stats *)malloc(sizeof(branch_stats)); 
                                    if(temp == NULL)
                                        cerr << "ERROR WHILE ALLOCATING BRANCH_STATS" << endl;
                                    temp->times_executed = 0;
                                    temp->taken = 0;
                                    temp->mispred = 0;
                                    current_branch = temp;
                                    branches_map[cpu].insert({arch_instr.ip, current_branch});
                                }
                                else{
                                    current_branch = it->second;
                                }
                                (current_branch->times_executed)++;

                                if (arch_instr.branch_taken)
                                    (current_branch->taken)++;
                            }
                            // END OF MY CODE
                        }
                        DP( if (warmup_complete[cpu]) {
